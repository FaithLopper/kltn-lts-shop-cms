...
class ProductUpdateForm extends BasicForm {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      nextProps.dataDetail.image !== this.state.image &&
      this.state.isUpdateLogo === false &&
      nextProps.dataDetail.image !== undefined
    ) {
      this.setState({
        image: `${AppConstants.contentRootUrl}${nextProps.dataDetail.image}`,
      });
    }
  }
      ...
      if (formValues.tags.length !== 0) {
        formValues.tags.map((item, index) => {
          ...
          if (
            ...
            index !== formValues.tags.length - 1
          )
            tags = tags.concat(" ");
  ...
  removeVariant = (index) => {
    ...
    this.setState({
      templateConfigData: templateConfigData.filter(
        (itemArray) => itemArray.index !== index
      ),
    });
    this.onValuesChange();
  };
    ...
    this.setState({
      templateConfigData: templateConfigData.map((item, index) => {
        if (item.index === this.selectedVariant) {
          return {
            ...item,
            variantIds: [...item.variantIds, variant],
          };
        }
        return item;
      }),
  ...
  formValidate() {
    const { templateConfigData } = this.state;
    if (templateConfigData.length === 0) return true;
  }
          ...
          {isTemplate ? (
            ...
            <VariantListForm
              ...
              selectedVariantArray={
                this.state.templateConfigData[this.selectedVariantArray]
                  ?.variantIds || []
              }
            />
          )}
