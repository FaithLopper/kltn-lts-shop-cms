...
class ProductChildListPage extends ListBasePage {
  ...
  getDetailLink(dataRow) {
    return sitePathConfig.productUpdate.path.replace(
      ":id",
      `${dataRow.id}?parentProduct=${this.parentProduct}`
    );
  }
  ...
  componentWillMount() {
    ...
    if (this.checkPermission()) this.loadDataTable(this.props);
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withTranslation(["productListPage", "listBasePage"])(ProductChildListPage));
